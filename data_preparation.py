...
#f = 'Total_dataset_JJ.xlsx'
    ...
    def plot_data(self):
        ...
        for out in self.all_outs:
            if out in self.args: obs_logy=True
            idx = 0
...
def assign_freq(df, index=None, file=None, force_freq=None,  verbosity=1, print_only = False):
    ...
    if verbosity > 0: print('in file {} frequency is {}'.format(file, df.index.freq))
    return df
...
def down_sample(data_frame, data_name, desired_freq, idx,  verbosity=1):
    ...
    if verbosity>1: print(data_frame.shape, data_frame.columns)
    ...
    if verbosity > 0: print('downsampling {} data from {} min to {}'.format(data_name, old_freq, out_freq))
    ...
    if data_name in ['air_temp_c', 'rel_hum', 'wind_dir_deg', 'wind_speed_mps', 'air_p_hpa', 'mslp_hpa',
        ...
        if verbosity>0: print('not resampling ', data_name)
        return data_frame
